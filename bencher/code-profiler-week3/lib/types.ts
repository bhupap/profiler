...
// Result of the "stuck or just slow?" watchdog run.
export type WatchdogVerdict = "completed" | "budget_exceeded" | "error";

export type WatchdogResult = {
  verdict: WatchdogVerdict;
  stepsUsed: number;        // how many steps before stopping
  stepBudget: number;       // the limit we set
  message: string;          // human-readable explanation
  // honest framing: budget_exceeded means "stuck OR just slow" — we can't decide which
};

// A file discovered in a GitHub repo tree.
