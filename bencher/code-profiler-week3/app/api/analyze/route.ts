...
export const maxDuration = 45; // WEEK 2: suggestions + flame graph take longer
...
export async function POST(req: NextRequest) {
  ...
  if (code.length > MAX_CODE_LENGTH)
    return NextResponse.json({ error: `Code exceeds ${MAX_CODE_LENGTH} characters` }, { status: 413 });
  if (!SUPPORTED.includes(language))
    return NextResponse.json({ error: `Language must be one of: ${SUPPORTED.join(", ")}` }, { status: 400 });
  if (!process.env.ANTHROPIC_API_KEY)
    return NextResponse.json({ error: "Server missing ANTHROPIC_API_KEY" }, { status: 500 });

  // WEEK 2: run the static pass first, feed results into the prompt.
  ...
  try {
    const resp = await client.messages.create({
      ...
      max_tokens: 3072, // bigger: now returning code suggestions too
